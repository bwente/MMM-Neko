...
const { spriteNames } = require("../lib/neko-engine.js");
const root = path.join(__dirname, "..");
const read = (name, mask) => {
  const source = fs.readFileSync(path.join(root, "assets/source", mask ? "bitmasks" : "bitmaps", `${name}${mask ? "_mask" : ""}.xbm`), "utf8");
...
spriteNames.forEach((name, frame) => {
  const bitmap = read(name, false), mask = read(name, true);
...
fs.writeFileSync(path.join(root, "assets/neko.svg"), `${svg}</svg>\n`);
